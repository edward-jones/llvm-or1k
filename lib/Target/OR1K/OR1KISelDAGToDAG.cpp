//===-- OR1KISelDAGToDAG.cpp - A DAG-DAG inst selector for OR1K -*- C++ -*-===//
//
...
#define DEBUG_TYPE "or1k-isel-dag-dag"
...
SDNode *OR1KDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::GLOBAL_OFFSET_TABLE:
    return SelectGLOBAL_OFFSET_TABLE(Node);
  case ISD::MULHU:
  case ISD::MULHS:
    return SelectMulHi(Node);
  case ISD::UMUL_LOHI:
  case ISD::SMUL_LOHI:
    return SelectMulHiLo(Node);
...
SDNode *OR1KDAGToDAGISel::SelectINTRINSIC_CHAIN(SDNode *Node) {
  ...
  default:
    return nullptr;
  case Intrinsic::or1k_mfspr:
    return SelectMFSPR(Node);
  case Intrinsic::or1k_mtspr:
    return SelectMTSPR(Node);
  }
}
